...
let myNFTstore, owner, addr1, addr2;

beforeEach(async function () {
    myNFTstore = await ethers.deployContract("myNFTstore");
    [owner, addr1, addr2, _] = await ethers.getSigners();
});

describe("Deployment", function () {
    it("Should set the right owner", async () => {
        const marketplaceOwner = await myNFTstore.marketplaceOwner();
        expect(marketplaceOwner).to.equal(owner);
    });

    it("Should set the listing fee percentage", async function () {
        const listingFeePercentage = await myNFTstore.getListingFeePercent();
...
describe("Creating NFTs", function () {
    it("Should create a new token and listing", async function () {
        ...
        await myNFTstore.createToken(tokenURI, price);

        const listing = await myNFTstore.getNFTListing(1);
...
describe("Updating Listing Fee", function () {
    it("Should update the listing fee percentage", async function () {
        await myNFTstore.updateListingFeePercent(10);
        expect(await myNFTstore.getListingFeePercent()).to.equal(10);
    });

    it("Should only allow owner to update the listing fee percentage", async function () {
        await expect(
            myNFTstore.connect(addr1).updateListingFeePercent(10)
...
describe("Executing Sales", function () {
    ...
    it("Should execute a sale", async function () {
        const price = ethers.parseEther("1");
        await myNFTstore.connect(addr1).executeSale(1, { value: price });
    ...
    it("Should revert if the payment is incorrect", async function () {
        ...
        await expect(
            myNFTstore.connect(addr1).executeSale(1, { value: incorrectPrice })
        ).to.be.revertedWith(
...
describe("Retrieving NFTs", function () {
    beforeEach(async function () {
        ...
        await myNFTstore.createToken(tokenURI1, price);
        await myNFTstore.createToken(tokenURI2, price);
    });

    it("Should retrieve all listed NFTs", async function () {
        const allNFTs = await myNFTstore.getAllListedNFTs();
        expect(allNFTs.length).to.equal(2);
    });

    it("Should retrieve my NFTs", async function () {
        const myNFTs = await myNFTstore.getMyNFTs();
